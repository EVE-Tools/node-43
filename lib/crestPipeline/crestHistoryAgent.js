...
// Metrics collection
var metricCrestCache = probe.metric({
  name  : 'CREST Cache Size',
  value : function() {
    return Object.keys(cache.buffer).length;
  }
});

var metricCrestCache = probe.metric({
  name  : 'CREST Cache Hit Rate',
  value : function() {
    if(cache.misses > 0) return cache.hits/cache.misses;
    return 0;
  }
});

var metricCrestQueue = probe.metric({
  name  : 'CREST Queue Length',
  value : function() {
...
var meterCrestRequests = probe.meter({name: 'CREST Requests/s'});
...
function handleResponse(task, callback, error, response, body) {
  ...
  if(typeof response !== 'undefined'){
    if (response.statusCode === 200) {
      ...
      axm.emit('crest:unknown-response', {
        status: response.statusCode,
