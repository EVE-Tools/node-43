  ...
  postgres(function deactivateOrders(err, pgClient, done){
    ...
    if (err) {
      ...
      pgClient.query('UPDATE market_data_orders ' +
                     'SET is_active = \'f\'' +
                     'WHERE mapregion_id=$1::int8 AND invtype_id=$2::int8', [resultSet.regionID, resultSet.typeID], function(err, result) {
...
function returnOrderID(order, index, objects){return order.orderID;}
...
function deactivateSelected(resultSet, callback){
  ...
  postgres(function deactivateSelectedOrders(err, pgClient, done){
    if (err) {
      ...
      // Execute query
      pgClient.query(query, [resultSet.regionID, resultSet.typeID].concat(objects.values), function(err, result) {
