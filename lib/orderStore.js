...
function executeQuery(resultSet, callback, results){
  ...
  postgres(function(err, pgClient, done){
    if (err) {
      ...
      pgClient.query(query, objects.values, function(err, result) {
        ...
        //if(err){ console.log(objects.values); console.log(resultSet);}
...
exports = module.exports = function(resultSet, callback){
  ...
  if (resultSet.type == 'history'){
    ...
    async.retry(3, executeQuery.bind(null, resultSet), function(err, result) {
